...
/* Transmit a number through USART 									*/
/* num is a number of type integer, and is what you want to send 	*/
void usart_transmit_number(int num) {
	char str[] = ""; //Create a string buffer 
	sprintf(str, "%d", num); //Concatenate string and number
	usart_transmit_string(str); //Send the newly created string
}
